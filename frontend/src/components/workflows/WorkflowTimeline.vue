  ...
  <div class="timeline-container pl-2">
    ...
    >
      ...
      <div class="me-3 position-relative z-index-1">
        ...
        >
          <VIcon
            :icon="getStatusConfig(step.status).icon"
            size="10"
          />
        </div>
      </div>
      ...
      <!-- Content -->
      <div
        class="d-flex flex-column pt-1 flex-grow-1"
        style="min-width: 0;"
      >
        <div class="d-flex justify-space-between align-start">
          <span
            class="text-subtitle-2 font-weight-bold text-truncate"
            :title="step.assignedUser?.name"
          >
            {{ step.assignedUser?.name || 'Unknown' }}
          </span>
          <span
            v-if="step.signed_at"
            class="text-[10px] text-medium-emphasis mt-1 text-no-wrap ms-2"
          >
        ...
        <!-- Decline Reason -->
        <div
          v-if="step.status === 'DECLINED' && step.decline_reason"
          class="mt-2 text-caption text-error bg-error-lighten-5 pa-2 rounded"
        >
          "{{ step.decline_reason }}"
        </div>
      </div>
